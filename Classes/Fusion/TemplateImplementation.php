<?php
namespace Vectoholic\Site\Fusion;
...
class TemplateImplementation extends OriginalTemplateImplementation
{
	...
	public function evaluate()
	{
        ...
        $componentsPath = 'resource://' . $packageKey . '/Private/Fusion/Components/' . $relativeName . '/' . $relativeName . '.' . $this->getTemplateType();
        $nodeTypesPath = 'resource://' . $packageKey . '/Private/Fusion/NodeTypes/' . $relativeName . '/' . $relativeName . '.' . $this->getTemplateType();

        $templatePath = $this->getTemplatePath();
        if ($templatePath === null) {
            if (file_exists($componentsPath)) {
                $templatePath = $componentsPath;
            } else if(file_exists($nodeTypesPath)) {
                $templatePath = $nodeTypesPath;
            } else {
                throw new \Exception(sprintf("
    				No template could be found for %s.
    				Make sure you defined one in your folder and check for misspellings too.
